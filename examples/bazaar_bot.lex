# examples/bazaar_bot.lex — an independent shopper agent (P2) for Bazaar Draft.
#
# Joins as P2 over A2A to receive a signed capability token, then on each of its
# turns drafts the best-value item it can still afford from the shared pool —
# submitting through the gated A2A endpoint. It can only act as P2, in turn.
#
# Env: SHOP_SERVER (default http://localhost:8900)
# Run: lex run --allow-effects env,io,net,time examples/bazaar_bot.lex run
...
# Best available item P2 can afford (max value); -1 if none.
type Best = { idx :: Int, val :: Int }
fn pick(pool :: List[jv.Json], budget :: Int) -> Int {
  let b := list.fold(pool, { idx: -1, val: -1 }, fn (acc :: Best, it :: jv.Json) -> Best {
    let owner := jstr(it, "owner")
    let price := jint(it, "price")
    let value := jint(it, "value")
    let i := jint(it, "i")
    if str.is_empty(owner) and price <= budget and value > acc.val { { idx: i, val: value } } else { acc }
  })
  b.idx
}
      ...
      if over { io.print("[P2] game over") } else {
        if turn == "P2" {
          ...
          if c < 0 {
            ...
            let _ := io.print(str.join(["[P2] drafting item ", int.to_str(c)], ""))
            let res := a2a(server, "shop_move", str.join(["{\"by\":\"P2\",\"item\":", int.to_str(c), ",\"token\":\"", token, "\"}"], ""))
...
fn run() -> [env, net, io, time] Unit {
  let server := match env.get("SHOP_SERVER") { None => "http://localhost:8900", Some(u) => u }
  let _ := io.print("══════════════════════════════════════════════════════")
  let _ := io.print("   BAZAAR P2-BOT  ·  independent shopper, drafts over A2A")
  let _ := io.print("══════════════════════════════════════════════════════")
  let joined := a2a(server, "shop_join", "{\"side\":\"P2\"}")
